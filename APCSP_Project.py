...
def want_a_workout_plan():
    while True:
        already_has_a_plan = input("Does your school already have a workout plan for you? (yes/no): ")
        if already_has_a_plan == "yes":
            another_plan = input("Do you want another workout plan? (yes/no): ")
            if another_plan == "yes":
                type_of_workout_plan = input("What type of workout plan do you want? [push/pull/legs/core/full body/cardio (type in the ones you want seperated by commas)]: ")
                workout_plan_wanted = [w.strip() for w in type_of_workout_plan.split(",")]
                for workout in workout_plan_wanted: 
                    if workout in workouts:
                        print(f"{workout}: {workouts[workout]}")
                        return f"This is your workout plan: {workout_plan_wanted}"
                break
        elif already_has_a_plan == "no":
            workout_plan = input("Do you want a workout plan? (yes/no): ")
            if workout_plan == "yes":
                return f"Here is your workout plan: {workout_plan_wanted}"
            elif workout_plan == "no":
                return "okay, but it's important to have one"
        result = want_a_workout_plan()
        print(result)
""
